...
int calc(int sd, int tm, int pon) {
		...
		for (i = 1; i <= sd; i++) {
			...
			ws = w; ms = pv[0][0];
			if (i >= 32000-w || sabort || t1 > searchtime/2) break;
			alpha = w - 60;
			beta = w + 60;
		}
		if (pon) return -1;
...
int main(int argc, char **argv)
{
	...
	_parse_fen(sfen);
	for (i = 0; i < 64; i++) nmobil[i] = bitcnt(nmoves[i]) << 3;
	for (i = 0; i < 64; i++) kmobil[i] = (bitcnt(nmoves[i])/2)*4;
	for (;;) {
